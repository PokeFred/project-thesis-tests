        ...
        {#each $game.stations as station}
            {#if station.state === "FINISHED"}
                <div class="w-full h-auto bg-green-500/15 border-2 border-green-500 rounded-xl grid grid-cols-[24px_auto_48px] gap-4 cursor-default p-4">
                    ...
                    <span class="text-left">{station.text}</span>
                    <span class="text-right">{station.points.current}/{station.points.max}</span>
                </div>
            {:else if station.state === "OPEN"}
                <button onclick={(): Promise<void> => goto(`./stations/${station.tag}`)} class="w-full h-auto border border-black rounded-xl grid grid-cols-[24px_auto_48px] gap-4 cursor-pointer p-4">
