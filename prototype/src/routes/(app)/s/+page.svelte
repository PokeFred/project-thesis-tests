...
<div class="w-full h-auto text-primary">
    ...
    <div class="mt-2 w-full h-auto grid grid-cols-1 gap-2">
        {#each data.stations as station}
            <button onclick={(): Promise<void> => goto(`/einkaufsspuren/s/${station.id}`)} class="w-full h-auto text-secondary bg-primary rounded-full grid grid-cols-[auto_60px] gap-4 cursor-pointer px-6 py-2 active:scale-95">
