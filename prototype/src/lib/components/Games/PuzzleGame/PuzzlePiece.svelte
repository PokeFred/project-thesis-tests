<script lang="ts">
    ...
    function inRange(data: DragEventData): boolean {
        ...
        const SLOT_DISTANCE_X = ((slotBbox.x * quiz.ImageWindow.ScaleWidth) - (LEFT_RELATIVE_TO_GAME));
        const SLOT_DISTANCE_Y = ((slotBbox.y * quiz.ImageWindow.ScaleHeight) - (TOP_RELATIVE_TO_GAME));              
        
        return Math.abs(SLOT_DISTANCE_X) < SNAP_RANGE && Math.abs(SLOT_DISTANCE_Y) < SNAP_RANGE;
    }

    function snap(data: any): void {
        piece.CurrentPosition = {x: (slotBbox.x * quiz.ImageWindow.ScaleWidth) - data.currentNode.offsetLeft, y: (slotBbox.y *  quiz.ImageWindow.ScaleHeight) - data.currentNode.offsetTop}
    }
...
<img
    ...
    style:width={piece.Dragging || piece.Placed ? `${slotBbox.width *  quiz.ImageWindow.ScaleWidth}px` : "100%" }
    style:height={piece.Dragging || piece.Placed ? `${slotBbox.height *  quiz.ImageWindow.ScaleHeight}px` : "100%"}
